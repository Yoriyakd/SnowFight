...
HedgeManager::HedgeManager(StageBorder * StageBorder)
{
	...
	{
		stageHedge.push_back(new StageHedge(&D3DXVECTOR3(stageBorder.Left - 2, 2.0f, (16.0f * i) + 8)));
	}

	for (int i = 0; i < FenceCnt; i++)		//stageRight
	{
		stageHedge.push_back(new StageHedge(&D3DXVECTOR3(stageBorder.Right + 2, 2.0f, (16.0f * i) + 8)));
	}
}
